...
Tripwire テスト用に吐く data-* 属性の命名は以下を指針とする（`data-token-*` / 旧 `data-divider` の命名揺れ整理は Issue #477 / PR #481 で確定済み、`data-tone` 等のドメイン固有 enum 属性は Issue #541 で規約化、`data-post-id` 等の識別子属性は Issue #696 で規約化）:
