...
新規コンポーネント追加時、`cva` recipe と `css()` + data-* 属性のどちらを使うかは以下で判断する（PR #474 で Button / Link が `cva`、他 11 コンポーネントが `css()` + data-* となった実例に基づく）：

- **`cva` recipe**: variant 数 ≥ 3 かつ atom コンポーネント。variant / size を recipe に集約する（例: `Button` = variant 3 × size 3、`Link` = variant 4）
- **`css()` + data-* 属性**: variant 数 ≤ 2、または atom 以外。recipe を起こさず `css()` のスタイルオブジェクトで十分（例: `BrandName` = variant 2）

#### data-* 属性命名規約

Tripwire テスト用に吐く data-* 属性は以下に統一する（命名揺れ整理は Issue #477 で確定）：

- **`data-token-*`**: Panda token 名のみを吐く（`accent.link` / `border.subtle` 等）。CSS キーワード（`inherit` 等）は吐かない。border 参照は方向問わず divider 含め `data-token-border` に統一する
- **CSS カスケード継承**: `inherit` 等のキーワードは `data-color-inherit="true"` のような bool 属性で表現し、token 参照（`data-token-*`）と区別する
