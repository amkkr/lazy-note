...
#### `heavy` 表示ポリシー (Issue #545 → Issue #839 で再評価・更新)

**現行ポリシー (Issue #839, 2026-06-28 以降)**:

`/anchor` ページは Footer の「サイトの読み方」入口リンクでナビ統合され
(Issue #839)、「能動的に URL を踏まないと到達しない補助ページ」から
**読者導線** へ性質が変化した。この性質変化を受けて heavy 表示ポリシーを
以下のとおり **更新** した:

- **公開 `/anchor` (読者面) はデフォルトで `tone === "heavy"` を抑制する**。
  `AnchorPage` の `showHeavy` prop の **デフォルトが `false` (抑制)** で、
  節目一覧と各記事の座標の両方を `src/lib/anchors.ts` の `excludeTones`
  (`HEAVY_TONES = ["heavy"]`) 経由で除外する (表示層に `tone === "heavy"` の
  enum 比較を持ち込まず、エンジン側の除外語彙を再利用する)。
- **隠す効果の限界 (誠実な明記)**: 本サイトは CSR の静的サイトで
  **GitHub Pages 配信 = サーバ側の認証境界を持てない**。URL 改変による再表示や、
  公開リポジトリの `datasources/milestones.json` 直読み・`git log` 閲覧は
  技術的に防げない。したがって heavy 抑制は **機密保護ではなく「重い節目を
  読者に不意に突きつけない配慮」** (UX 上の配慮) であって、秘匿措置ではない。
- **運営者の全件確認**は、ローカル開発で `AnchorPage` に `showHeavy={true}` を
  渡すか、リポジトリの `datasources/milestones.json` を直読みして行う
  (公開ページの出し分けには依存しない)。
- **URL クエリ (`/anchor?showHeavy=...`) による出し分けは実装しない**。
  prop default を抑制側に倒すことで読者面を抑制し、運営者面はローカルに閉じる。
- **クローラ制御**: `/anchor` は `<meta name="robots" content="noindex,follow">`
  を出力して検索インデックス対象から外す (`/` ・ `/posts/:timestamp` には
  robots を付けずインデックスを維持する)。これはクローラ流入を緩和する主装置
  だが、**CSR 注入ゆえの限界** がある: 本サイトは CSR (GitHub Pages 配信) で、
  robots meta は React 描画後に client-side で `<head>` へ注入される。JS を
  実行して描画するクローラ (Googlebot 等) には届くが、**JS を実行しない
  クローラや raw HTML を取得するスクレイパは初期静的シェル (robots タグなし)
  を見るため `noindex` が届かない**。したがって robots も「完全秘匿」ではなく
  「JS 描画クローラ向けの緩和策」であり、上記「機密保護ではなく配慮」トーンと
  同様に過剰な効果を約束しない。

> **歴史的経緯 (Issue #545 当時の判断)**: 以下は #545 当時、`/anchor` を「能動的
> に URL を踏まないと到達しない補助ページ」と位置付けていた前提での判断
> (「公開 `/anchor` でも heavy を全件表示する」) であり、`milestones.json` が
> 公開リポジトリで管理されている defensible 性に基づく **暫定的判断** だった。
> #545 の注記自身が「将来の利用実態で再評価が必要になった場合は撤退戦略の
> いずれかへ切り替える」と明記しており、**Issue #839 がまさにその再評価**として
> 撤退戦略 #3 (`showHeavy`) を稼働させた (下記「撤退戦略」#3 参照)。#545 の
> 判断根拠のうち「3. 読者導線の主面ではない」は性質変化により **失効** し、
> これが読者面でのデフォルト抑制へ結論が反転した直接の理由である。残る
> 「1. 公開リポジトリで既に公開済み」は上記「隠す効果の限界」の根拠として、
> 「2. 運用画面の透明性」は `showHeavy={true}` の運営者全件確認パスの根拠として
> 引き続き有効。
...
| **AnchorPage 読者面 (既定)** | **抑制する** (`showHeavy=false`)       | 読者導線化 (#839) に伴い不意に突きつけない配慮      |
| **AnchorPage 運営者面**      | **表示する (全件)** (`showHeavy=true`) | ローカルでの全件確認 = 運用画面の透明性 (#545 由来) |
...
したがって AnchorPage は「3 つの面のうち、(運営者面 = `showHeavy={true}` の
とき) heavy を全件出しうる唯一の面」となる。この非対称性は **意図的** で、
Coordinate / Resurface との一貫性違反ではない。**読者面 (既定 =
`showHeavy=false`) では他の 2 面と同様に heavy を抑制する** (Issue #839 で
読者導線化したため。上記「現行ポリシー」を参照)。
...
3. **`AnchorPage` の `showHeavy` prop による表示制御 (Issue #839 で稼働中)**:
   案 1 / 2 で網羅できないユースケース「heavy を持つ運用は続けたいが、読者面
   では隠したい」に対応する手段。**当初 (#545 時点) は「将来の選択肢、現時点
   では実装しない (YAGNI)」と位置付けていたが、Issue #839 で `/anchor` が
   読者導線化したのを機に稼働させた**。実装は `AnchorPage` に
   `showHeavy?: boolean` prop を追加し、**デフォルトを `false` (= 抑制) に
   倒した** (#545 当時の実装イメージはデフォルト `true` = 現状維持だったが、
   読者面抑制を既定にするため反転している点に注意)。公開 `pages/anchor.tsx`
   は `showHeavy` を渡さない (= 読者面は heavy 抑制) で運用し、運営者の全件
   確認は `showHeavy={true}` をローカルで渡すか `milestones.json` 直読みで
   行う。URL クエリ (`/anchor?showHeavy=false`) による出し分けは **採用
   しない** (= prop default の切替に一本化し、読者面 / 運営者面を URL 状態に
   持たせない)。
...
- **ルーティングからリンクを外す**: Footer の「サイトの読み方」入口リンク
  (`src/components/layouts/Footer.tsx`、Issue #839 で追加) を削除する
  (= ページ自体は残るが読者導線が無くなる)。リンクを外しても `/anchor` は
...
## 見送り中フィーチャの再評価トリガー (Issue #840)

「記事末尾に座標近接ベースの関連記事を出す」フィーチャは、現在の節目構成では
PostNavigation (前後記事ナビ) の劣化コピーになるため見送り中。見送り前提が
崩れたことを機械的に検知するトリガー検知器として `src/lib/anchorTriggers.ts`
(純粋関数) と実データ Tripwire
`src/lib/__tests__/anchorTriggers.allPosts.test.ts` を置いている。検知は 3 経路:

- **maxShare < 50%**: 各記事の支配節目 (heavy 除外後・最後に通過した節目) の、
  **支配節目を持つ記事のうち** 最大グループ比率が 50% 未満になったとき
  (現状は全 `.md` が支配節目を持つため「全記事のうち」と一致)。
- **母数乖離**: 母数 (支配節目を持つ記事数) が全記事数と乖離したとき
  (過去記事バックフィル等で「全記事が支配節目を持つ」前提が崩れたサイン)。
- **非 heavy 節目 3 件化**: 非 heavy 節目が 2 件しかない現構造では maxShare が
  常に 50% 以上で vacuous (経路 1 が死ぬ) なため、3 件以上になったら即検知する。

いずれかが fail するとフィーチャ再評価のサインになる (2026-06 時点の maxShare は
約 77.8%)。
...
- [`docs/MARKDOWN_LINT.md`](./MARKDOWN_LINT.md): 本ドキュメント内の表セル幅
  整列を手動運用にしている理由と、手動 drift 検知のセルフチェック手順
