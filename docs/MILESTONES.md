...
ランタイム検証は `src/lib/milestonesSchema.ts` の `parseMilestones` /
`validateMilestonesStrict` で行う (Issue #547)。

### フィールド

- **`date`**: `YYYY-MM-DD` (JST 想定)。形式違反の要素はランタイム検証で除外される
- **`label`**: 表示用ラベル (例: `"社会復帰"`、`"サイト開設"`)。空文字列の要素は
  ランタイム検証で除外される
- **`tone`**: 表示時の扱いを切り替える感情タグ。3 値 (`neutral` / `light` /
  `heavy`) 以外は除外される。次節で詳述する

**経路ごとの検証動作 (Issue #547 で統一)**:

`milestones.json` を読む経路は 2 系統あるが、検証ロジックは
`src/lib/milestonesSchema.ts` の `parseMilestones` に集約済みで、
不正値の扱いは両経路で完全に同一である。

- **Cast (`scripts/newPost.ts` の `loadMilestones`)**: 内部で `parseMilestones`
  を呼び、日付形式違反 / `tone` 不正値 / `label` 空 / 型不正のエントリを
  **全て除外する**
- **Coordinate / Resurface / AnchorPage (`pages/index.tsx` /
  `pages/posts/Post.tsx` / `pages/anchor.tsx`)**: JSON import 直後に
  `parseMilestones(milestonesData)` で narrowing。型キャストではなく
  ランタイム検証を経由するため、**`tone` 不正値もランタイムで除外される**

**CI による検出**: `src/lib/__tests__/milestonesSchema.test.ts` の
「datasources/milestones.json の実データ検証」テスト群が
`validateMilestonesStrict` で本番 JSON を **strict** に検証する。
`tone: "happy"` のような値域外の値を含む PR は CI が落ちて merge 前に
ブロックされる。CI ログには「どの index のどのフィールドがどう不正か」が
全件表示されるため、複数箇所の不正を 1 回で修正できる。

**運用上の注意**: 上記の通り `tone` の 3 値外はランタイムで除外されて
画面には出ないが、**沈黙的に消える** ため運営者が間違いに気付かないリスクは残る。
`tone` は必ず 3 値 (`neutral` / `light` / `heavy`) 内で書くこと。
PR 時点では CI の `validateMilestonesStrict` テストが検出する。
