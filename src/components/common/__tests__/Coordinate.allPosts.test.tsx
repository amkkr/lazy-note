...
/**
 * Issue #491 AC「既存の全記事で Coordinate が正しく表示されること、
...
describe("Coordinate (実データ全記事での回帰テスト)", () => {
  ...
  describe("0 件記事の非表示 (フォールバック)", () => {
    it("milestones が空配列なら全記事で非表示になる", () => {
    ...
    it("全節目が tone:heavy だけなら全記事で非表示になる", () => {
    ...
    it("全節目が publishedAt より未来なら全記事で非表示になる", () => {
      // 既存記事のうち最古は 20250826 のため、それより未来の節目を作る
