...
describe("selectResurfaced: 既存17記事の実データに対する分岐", () => {
  /**
  ...
  it("today=2026-08-22 では最新記事 (2026-06-14) から 69 日経過のため沈黙トリガーが発火する", () => {
  ...
  it("today=2026-06-15 (最新記事翌日) では沈黙していないが、1年前 (2025-06-15) の記事も無いため null", () => {
    const posts = loadRealPostSummaries();
  ...
  it("today=2026-08-26 (沈黙 & 1年前 2025-08-26 に記事あり) では沈黙が暦の節目より優先される", () => {
    const posts = loadRealPostSummaries();
    // 最新の 2026-06-14 から today までの日数を確認
    // 2026-06-14 から 2026-08-26 は 73 日経過 → 沈黙トリガー (>=30) が先に発火
...
describe("selectResurfaced: options.excludeIds (View Transition 名前衝突回避)", () => {
  ...
  it("実データ (17記事) で today=2026-08-22、最古記事 id を excludeIds に渡すと別記事 or null になる", () => {
    // datasources から実 17 記事を読む。最古は 20250826031705。
